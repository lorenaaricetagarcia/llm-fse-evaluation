#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
RAG with PubMed – v1 (dynamic model)
Author: Lorena Ariceta García
"""

import os, sys, json, re, time, requests, spacy, xml.etree.ElementTree as ET, pandas as pd, argparse
from keybert import KeyBERT
from sentence_transformers import SentenceTransformer
from datetime import datetime
from transformers import pipeline
from utils.load_model import load_hf_model

# Paths and imports
BASE_DIR = "/home/xs1/Desktop/Lorena/MEDICINA"
if f"{BASE_DIR}/code/3_RAG" not in sys.path:
    sys.path.append(f"{BASE_DIR}/code/3_RAG")
sys.path.append(f"{BASE_DIR}/code/3_RAG")
from prompt_config import PROMPTS

# Args / env
parser = argparse.ArgumentParser(description="RAG PubMed v1 – multilingual")
parser.add_argument("--lang", choices=["es", "en"], default="es")
args = parser.parse_args()

LANG = os.getenv("RAG_LANG", args.lang)
MODEL_NAME = os.getenv("RAG_MODEL")
assert MODEL_NAME, "❌ RAG_MODEL not defined"

PROMPT_RAG = PROMPTS[LANG]
EXAMS_DIR = f"{BASE_DIR}/results/1_data_preparation/6_json_final/prueba"
OUTPUT_DIR = f"{BASE_DIR}/results/2_models/2_rag/2_pubmed/v1_{LANG}"
os.makedirs(OUTPUT_DIR, exist_ok=True)
LOG_FILE = os.path.join(OUTPUT_DIR, f"rag_pubmed_v1_{MODEL_NAME}_{LANG}_log.txt")

class DualLogger:
    def __init__(self, path): self.terminal=sys.stdout; self.log=open(path,"w",encoding="utf-8")
    def write(self,msg): self.terminal.write(msg); self.log.write(msg)
    def flush(self): self.terminal.flush(); self.log.flush()
sys.stdout = DualLogger(LOG_FILE)

print(f"🧠 MODEL: {MODEL_NAME} | 🌐 LANG: {LANG} | ⏳ {datetime.now():%Y-%m-%d %H:%M:%S}")

sentence_model = SentenceTransformer("all-MiniLM-L6-v2")
kw_model = KeyBERT(model=sentence_model)
try: nlp = spacy.load("en_core_web_sm")
except: nlp = spacy.load("es_core_news_sm")

# PubMed
def get_keywords(text):
    try: return [k[0] for k in kw_model.extract_keywords(text, top_n=3)]
    except: return [t.text for t in nlp(text) if t.pos_ in ["NOUN","PROPN"]][:3]

def retrieve_pubmed_context(query, top_k=3):
    kw = get_keywords(query)
    if not kw: return "No keywords found.", []
    term = "+".join(kw)
    try:
        ids = [e.text for e in ET.fromstring(requests.get(
            f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term={term}&retmax={top_k}"
        ).text).findall(".//Id")]
        if not ids: return "No PubMed results.", []
        txt = requests.get(f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id={','.join(ids)}&retmode=text&rettype=abstract").text
        return txt[:2000] if txt else "Empty abstracts.", ids
    except Exception as e:
        return f"⚠️ PubMed error: {e}", []

def generate_answer(prompt):
    try:
        if MODEL_NAME.startswith("med") or MODEL_NAME.lower() in ["llama3","mistral","gemma"]:
            r=requests.post("http://localhost:11434/api/generate",json={"model":MODEL_NAME,"prompt":prompt,"stream":False},timeout=180)
            return r.json().get("response","").strip()
        else:
            pipe=load_hf_model(MODEL_NAME)
            return pipe(prompt,max_new_tokens=256,do_sample=False)[0]["generated_text"].strip()
    except Exception as e:
        return f"❌ {e}"

# Main
metrics=[]
for f in os.listdir(EXAMS_DIR):
    if not f.endswith(".json"):continue
    tit=f.split("_")[0]
    with open(os.path.join(EXAMS_DIR,f),"r",encoding="utf-8") as g:data=json.load(g)
    preguntas = [
        q for q in data.get("preguntas", [])
        if not q.get("tipo") or q.get("tipo").lower() in ["texto", "text", "teorica"]
    ]
    results=[]
    for q in preguntas:
        ctx, ids = retrieve_pubmed_context(q["enunciado"])
        opts="\n".join(f"{i+1}. {o}" for i,o in enumerate(q["opciones"]))
        prompt=f"{PROMPT_RAG}\n📚 CONTEXT (PubMed):\n{ctx}\n❓ {q['enunciado']}\n{opts}"
        out=generate_answer(prompt)
        m=re.search(r"\b([1-4])\b",out)
        pred=int(m.group(1)) if m else None
        results.append({"numero":q["numero"],"pmids":ids,"respuesta_correcta":q["respuesta_correcta"],MODEL_NAME:pred})
    acc=sum(1 for r in results if r[MODEL_NAME]==r["respuesta_correcta"])/max(1,len(results))*100
    metrics.append({"Modelo":MODEL_NAME,"Titulación":tit,"Accuracy (%)":round(acc,2)})
    json.dump({"preguntas":results},open(os.path.join(OUTPUT_DIR,f"{tit}_{MODEL_NAME}_rag_pubmed_v1_{LANG}.json"),"w",encoding="utf-8"),ensure_ascii=False,indent=2)

pd.DataFrame(metrics).to_excel(os.path.join(OUTPUT_DIR,f"rag_pubmed_v1_{MODEL_NAME}_{LANG}_metrics.xlsx"),index=False)
print("🏁 Completed.")
