...
# Traductor ligero Helsinki
try:
    translator = pipeline(
        "translation",
        model="Helsinki-NLP/opus-mt-es-en",
        device=-1  # fuerza CPU, más estable
    )
...
def translate_keywords_es_en(kws_es):
    """Traduce lista de keywords al inglés usando Helsinki."""
    ...
    try:
        ...
        if len(parts) < 3:
            # fallback: traducir palabra a palabra
            parts = []
            for k in kws_es:
                try:
...
for modelo in modelos:
    for titulacion, preguntas in resultados_titulacion[modelo].items():
        salida_json = os.path.join(carpeta_salida, f"{titulacion}_{modelo}_RAG_PubMed_live_v2.json")
    ...
    for modelo in modelos:
        ...
        for titulacion, preguntas_modelo in resultados_titulacion[modelo].items():
            ...
            with open(ruta_correctas, "r", encoding="utf-8") as f_corr:
                ...
                correctas_dict = {p["numero"]: p.get("respuesta_correcta") for p in data_corr["preguntas"]}
