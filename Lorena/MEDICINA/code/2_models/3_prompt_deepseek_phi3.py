...
carpeta_salida = "/home/xs1/Desktop/Lorena/results/2_models/1_prompt/3_prompt_deepseek_phi3"
os.makedirs(carpeta_salida, exist_ok=True)

# 🔄 Redirección de salida (pantalla + archivo)
class DualOutput:
...
# Redirigir salida
sys.stdout = DualOutput(os.path.join(carpeta_salida, "log_prompt_es_phi3_deepseek.txt"))
...
for archivo_json in archivos_json:
    ...
    # Duplicados en "numero"
    numeros = [p.get("numero") for p in base_data["preguntas"]]
    dup_count = sum(1 for _, c in Counter(numeros).items() if c > 1)
    if dup_count > 0:
        print(f"⚠️ {archivo_json}: detectados {dup_count} duplicados en 'numero' → se usará comparación por posición.\n")
    ...
    for modelo, modelo_interno in modelos.items():
        ...
        for i, pregunta in enumerate(base_data["preguntas"], 1):
            ...
            payload = {"model": modelo_interno, "prompt": prompt, "stream": False}
        ...
        print(f"📈 Porcentaje acierto   : {acierto_pct:.2f}%")
...
csv_path = os.path.join(carpeta_salida, "prompt_es_phi3_deepseek_metrics.csv")
excel_path = os.path.join(carpeta_salida, "prompt_es_phi3_deepseek_metrics.xlsx")
...
for modelo in modelos:
    ...
    print(f"Errores              : {errores}")
    print(f"No respondió (None)  : {sin_respuesta}")
    print(f"📈 Accuracy (%)       : {acierto_pct:.2f}%")
