# ================================================================
# RAG con abstracts de PubMed (consulta en tiempo real)
# ================================================================
...
carpeta_salida = "results/2_models/rag_pubmed_v1_09102025"
...
def retrieve_pubmed_context(query, top_k=3):
    ...
    try:
        # Extraer 3 palabras clave principales
        ...
        # Buscar artículos -> obtener PMIDs
        url_search = (
            f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi"
            f"?db=pubmed&term={search_term}&retmax={top_k}"
        )
        search_resp = requests.get(url_search)
        tree = ET.fromstring(search_resp.text) # Convierte el XML en un árbol 
        ids = [id_elem.text for id_elem in tree.findall(".//Id")] # Extrae texto dentro de cada nodo --> lista con PMIDs
        ...
        fetch_resp = requests.get(url_fetch) # Llam a la API para descargar los abstracts
        abstracts = fetch_resp.text.strip()

        # Limitar a 3 abstracts o 2000 caracteres
...
for archivo_json in archivos_json:
    ...
    for modelo in modelos:
        ...
        for i, pregunta in enumerate(base_data["preguntas"], 1):
            ...
            # 🔍 Recuperar contexto desde PubMed
            ...
            PROMPT_RAG = (
                ...
                "Primero analiza la información recuperada de la literatura biomédica (PubMed abstracts) y luego responde.\n"
                "Si el contexto es relevante, úsalo para justificar la elección.\n"
                "Responde con el formato: 'La respuesta correcta es la número X.' (X entre 1 y 4), "
                "seguido de una breve justificación médica.\n"
                "No inventes ni des varias opciones. Sé preciso y clínicamente correcto.\n\n"
            )
            ...
            # 🚀 Llamada al modelo vía Ollama API local
            try:
...
carpeta_salida_abs = os.path.join(os.getcwd(), carpeta_salida)
os.makedirs(carpeta_salida_abs, exist_ok=True)

print("\n📊 Calculando métricas finales comparando con respuestas correctas...")

# Crear CSV resumen
csv_path = os.path.join(carpeta_salida_abs, "resumen_accuracy_rag_pubmed_v1.csv")
with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
    ...
    for modelo in modelos:
        ...
        for titulacion, preguntas_modelo in resultados_titulacion[modelo].items():
            # Buscar archivo de respuestas correctas
            ...
            with open(ruta_correctas, "r", encoding="utf-8") as f_corr:
                ...
                correctas_dict = {p["numero"]: p.get("respuesta_correcta") for p in data_corr.get("preguntas", [])}

            total = len(preguntas_modelo)
            aciertos = 0
            errores = 0
            sin_contestar = 0

            for p in preguntas_modelo:
                num = p.get("numero")
                seleccion = p.get(modelo)
                correcta = correctas_dict.get(num)
